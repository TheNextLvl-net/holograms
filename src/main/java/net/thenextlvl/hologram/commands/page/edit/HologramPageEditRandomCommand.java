...
@NullMarked
public final class HologramPageEditRandomCommand extends BrigadierCommand {
    private HologramPageEditRandomCommand(final HologramPlugin plugin) {
        super(plugin, "random", "holograms.command.page.edit.random");
    }

    public static LiteralArgumentBuilder<CommandSourceStack> create(final HologramPlugin plugin) {
        final var command = new HologramPageEditRandomCommand(plugin);
        return command.create()
                .then(Commands.argument("random", BoolArgumentType.bool())
                        .executes(command::setRandom));
    }
    ...
    private int setRandom(final CommandContext<CommandSourceStack> context) {
        ...
        plugin.bundle().sendMessage(context.getSource().getSender(), "hologram.page.random",
                ...
                Placeholder.unparsed("random", String.valueOf(random)));
