...
import net.thenextlvl.dialogs.input.TextInput;
...
final class CreateHologramDialog {
    ...
    static Dialog<?> create(final String initial, @Nullable final Component note) {
        ...
        return dialog.addInput(hologramNameInput(initial))
                .addButton(createHologramButton())
                .exitAction(BackButton.create(ignored -> OverviewDialog.create()).width(150));
    }

    private static CallbackButton createHologramButton() {
    ...
    private static TextInput hologramNameInput(final String initial) {
        return Input.text("name", Component.text("Hologram name"))
                .initial(initial);
    }
}
