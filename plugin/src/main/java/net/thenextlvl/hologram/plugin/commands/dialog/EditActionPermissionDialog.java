...
final class EditActionPermissionDialog {
    ...
    static net.thenextlvl.dialogs.Dialog<?> create(
        ...
        body.add(Body.text(Component.text("Set a permission or leave it blank to clear it")));
        if (note != null) body.add(Body.text(note));
        final var dialog = Dialog.multiAction().title(Component.text("Permission"));
        body.forEach(dialog::addBody);
        List.of(Input.text("permission", Component.text("Permission")).initial(current).build()).forEach(dialog::addInput);
