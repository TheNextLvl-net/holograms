...
@NullMarked
final class EditScaleDialog {
    private EditScaleDialog() {
    }
    ...
    static net.thenextlvl.dialogs.Dialog<?> create(
        ...
        final var save = Button.callback((response, audience) -> {
            final var parsed = DialogSupport.parseDouble("Scale", response.getText("value"), min, max);
            if (parsed.error() != null) {
                DialogSupport.show(audience, ignored -> EditScaleDialog.create(current, min, max, Component.text(parsed.error(), NamedTextColor.RED), setter, reopen));
                return;
            }
            if (!Double.isFinite(parsed.value())) {
                DialogSupport.show(audience, ignored -> EditScaleDialog.create(current, min, max, Component.text("Scale must be a finite number", NamedTextColor.RED), setter, reopen));
                return;
            }
        ...
        body.add(Body.text(Component.text("Enter a scale value between 0.1 and 100")));
        ...
        List.of(Input.text("value", Component.text("Scale")).initial(Double.toString(current)).build()).forEach(dialog::addInput);
