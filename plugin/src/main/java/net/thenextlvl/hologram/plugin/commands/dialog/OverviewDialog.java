...
final class OverviewDialog {
    ...
    public static net.thenextlvl.dialogs.Dialog<?> create() {
        ...
        final var create = Button.clickEvent(ClickEvent.callback(audience -> {
        ...
        holograms.stream().map(hologram -> Button.clickEvent(ClickEvent.callback(audience -> {
            DialogSupport.show(audience, viewer -> EditHologramDialog.create(hologram, viewer));
        }), Component.text(hologram.getName()))).forEach(actions::add);
        actions.add(create);

        final var body = new ArrayList<DialogBody>();
        if (holograms.isEmpty()) {
            body.add(Body.text(Component.text("No holograms have been created yet")));
        }

        final var dialog = Dialog.multiAction().title(Component.text("Manage your Holograms"));
