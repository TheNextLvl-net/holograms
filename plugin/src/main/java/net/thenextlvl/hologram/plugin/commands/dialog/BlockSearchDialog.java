...
final class BlockSearchDialog {
    ...
    static Dialog<?> create(
            ...
            final List<? extends Button<?>> extraActions,
            final Button<?> back,
    ...
    static Dialog<?> create(
        ...
        if (currentPage > 0) actions.add(DialogSupport.pageButton("Previous Page", audience -> {
            return BlockSearchDialog.create(title, initial, note, extraActions, back, selection, currentPage - 1);
        }));
        ...
        dialog.addInput(Input.text("search", Component.text("Search")).initial(initial));
        actions.forEach(dialog::addButton);
        return dialog.exitAction(back);
    }
}
